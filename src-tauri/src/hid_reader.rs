...
#[derive(Serialize, Clone, Debug)]
pub struct HidButtonReport {
    pub pressed_buttons: Vec<u32>, // List of button indices that are currently pressed (1-based)
    pub timestamp_ms: u64,
}
...
pub fn parse_hid_axes_with_descriptor(
    report: &[u8],
    device_path: &str,
    ...
    for field in input_report.fields() {
        match field {
            Field::Variable(var) => {
                ...
                // The extract method returns a FieldValue which can be converted to i32
                match var.extract(report) {
                    Ok(field_value) => {
                        // FieldValue implements Into<i32> - this preserves proper value ranges
...
pub fn parse_hid_axes(
    report: &[u8],
    axis_names: &HashMap<u32, String>,
) -> Result<HidAxisReport, String> {
    let mut axis_values = HashMap::new();
    ...
    eprintln!(
        "[HID] Axis data starts at offset {}, {} bytes remaining",
        axis_start_offset,
        remaining.len()
    );
    ...
    if is_16bit {
        ...
        for i in (0..remaining.len()).step_by(2) {
            if i + 1 < remaining.len() {
                ...
                eprintln!(
                    "[HID] Axis {} (16-bit): {} ({}-bit precision, bytes: 0x{:02X} 0x{:02X})",
                    axis_id,
                    value,
                    bit_depth,
                    remaining[i],
...
fn detect_16bit_axes(remaining: &[u8]) -> bool {
    ...
    if remaining.len() < 4 {
        eprintln!(
            "[HID] Too few bytes ({}) for 16-bit detection, defaulting to 8-bit",
            remaining.len()
        );
        return false;
    }
    ...
    if remaining.len() >= 8 && remaining.len() % 2 == 0 {
        eprintln!(
            "[HID] {} bytes (even count >= 8), assuming 16-bit",
            remaining.len()
        );
        return true;
    }
    ...
    if remaining.len() >= 4 && remaining.len() % 2 == 0 {
        eprintln!(
            "[HID] {} bytes (even count >= 4), assuming 16-bit",
            remaining.len()
        );
        return true;
    }
...
mod tests {
    ...
    fn test_parse_simple_report() {
        ...
        let axis_names = HashMap::new();
        let result = parse_hid_axes(&report, &axis_names).unwrap();
